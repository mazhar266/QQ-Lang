...
v1 is complete: lexer, parser, AST, error model, source registry, `Repository` cache, Quran resolver, hadith resolvers (16 collections), Hisnul Muslim resolver, user-defined JSON sources, book-wide `B::100` numbering, full-text search, `qql` CLI, C ABI (`src/ffi.rs` + `include/qql.h`), Dart binding, fuzz targets, CI. 120 Rust tests plus 13 Dart tests pass; 155 with `--features vector,fulltext`. `docs/plan.md` is the spec (41 sections, 11 phases) and remains the authority on design.
...
```bash
python3 scripts/build-vectors.py                       # sources/vectors/*.qv  (committed, ~90 s, 33 MB)
...
- Matching tries the needle against a record's `ar`, `emlaei` *and* `en`, so one term searches both spellings and both languages. It is plain substring, not stemming: `"mercy"` does not match *Merciful*. A **single-word** needle also tries its `search::aliases` group; a multi-word needle is a literal substring and is never rewritten.
...
- **Embedder 2 (`HashedIdf`) is what the shipped indexes use.** Tokens are scaled by IDF, whole words by `WORD_WEIGHT` (3.0) against a trigram's 1.0, and stopwords are zeroed. Embedder 1 is kept and still parses — `project()` takes the weight closure so `hashed_embed` stays bit-identical to what it always produced. `tests/vector.rs::the_build_script_and_the_runtime_embed_alike` pins the cross-language invariant by re-embedding a stored record and demanding self-similarity > 0.98; that is the test that catches drift between the Rust and Python embedders.
- The weight table ships **inside the `.qv`**, keyed by token hash so no vocabulary is needed: `f32 default` then sorted `(u64 hash, f32 idf)` pairs after the vectors, count in the old `reserved` field at offset 20 (always 0 before, so embedder-1 files stay valid). Tokens with `df == 1` are omitted — they all take `default`, which is most of the vocabulary, so the table is ~0.3 MB per source. Parsing rejects an unsorted table, since the lookup binary-searches it.
- `src/stopwords.txt` is **single-sourced**: `include_str!` in `src/search.rs`, read by path in `scripts/build-vectors.py`. It had to be shared once the build script started zeroing stopwords — a list that drifted between the two sides would change every score silently. Do not inline it back into either.
...
- The tantivy index stores **every** field folded (`search::fold`), English included, then runs English through `en_stem` — that stemmer is why `?"mercy"` finds *Merciful* where `"mercy"` cannot. English was indexed raw until the apostrophe fix; folding only the query would not have helped, since the mismatch is on both sides.
- **`fold` drops the apostrophe class** (`'` `‘` `’` backtick `ʼ` `ʻ` `ʾ` `ʿ`). Every tokenizer splits on non-alphanumerics, so `Qur'an` indexed as `qur` + `an` and `?"quran"` matched nothing. The corpus mixes spellings itself: `qur'an` ×199 vs `qur’an` ×50, `` `Asr `` with a backtick. Measured before choosing the set — U+2018 and U+0060 each appear ~720 times, so the user's original four would have left holes.
- **Full-text terms are OR, not AND.** `set_conjunction_by_default()` is gone: one unmatched word used to return zero results for a whole phrase. BM25 does the min-should-match job, and IDF makes a surviving stopword nearly weightless. An explicit `AND` in the term still reaches tantivy.
- **Query rewriting is guarded by `search::is_plain`** — no phrase, boolean, field or sign syntax. Only plain terms get stopwords dropped and aliases expanded; rewriting inside `"the straight path"` would change the question. `fulltext::rewrite` emits `(a OR b OR c)` per aliased word; `Context::prepare` appends variants for the embedder.
- `search::ALIASES` is ~35 curated groups, **query-time only** — no index rebuild when it changes, which is why it lives here rather than in `fold`. Entries are written folded (so `qur'an` is absent, it already folds to `quran`), and collision-prone pairs (`lut`/`lot`, `ayyub`/`job`) are deliberately excluded.
...
- **`Record::emlaei` is the one text field beyond `ar`/`en`**, and the one exception to "anything else goes in `extra`" — search is source-agnostic, so a second spelling had to be a field rather than a key the search path would have to know by name. Empty for every source but the Quran, and `skip_serializing_if` keeps it out of their JSON. The Quran's `ar` stays Uthmani; `emlaei` is the simplified spelling from the committed `sources/hafs_smart_v8.json`, merged in by `scripts/build-quran.py`.
- **Folding cannot bridge the two rasms** — `ٱلصَّلَوٰةَ`/`الصلاة`, `ٱلْكِتَٰب`/`الكتاب` differ by a letter, not a mark — which is why the second spelling is carried rather than normalized at query time. All three engines search it: `Context::exact` matches it, `fulltext` indexes it as its own `emlaei` field, vectors emit `LANG_EM = 2`. Note the supplied text keeps some classical spellings (`السموات`, not `السماوات`).
- `fulltext::Searcher::open` resolves field handles from the **index's own schema by name**, not from a freshly built one — a `Field` is just a position, so adding a field would otherwise misaddress every index already on disk. `emlaei` is `Option<Field>`, so an index built before it existed still opens.
- `build-quran.py` guards the pairing: with alef/waw/ya stripped, a correctly aligned ayah matches its Emlaei spelling 98.7% of the time and a shifted one 0%, so the 5% limit is a wide margin. `--meta` is required; run it with a `quran-json-arabic` checkout.
- `search::fold` also drops invisible joiners and bidi marks (`U+200C..U+200F`, `U+061C`) — five Emlaei ayat carry a stray `U+200E`. `scripts/build-vectors.py` mirrors this; **the two folds must stay identical**.
...
- **`DA` (ad-Darimi) has no English upstream** — all 2757 records have an empty `en`, so an English search finds nothing there. The Muwatta has 125 entries with English but no Arabic. Both fields always exist; an untranslated one is `""`, never a missing key.
